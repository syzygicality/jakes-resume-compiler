...
func compileCall(url string, source []byte, apiKey string) (int, error) {
...
func TestHTTPCompile(url string, apiKey string, concurrency int, totalRequests int) {
	fmt.Println("URL:", url)
	...
	for range totalRequests {
		wg.Go(func() {
			...
			reqStart := time.Now()
			statusCode, err := compileCall(url, body, apiKey)
